    ...
    <div className={cn("relative", className)}>
      <div className="flex justify-between items-center mb-4">
        ...
        {viewAllLink && (
          <a 
            href={viewAllLink} 
            className="text-sm font-medium text-book-700 hover:text-book-800 transition-colors"
          >
            {t('section.viewAll')}
          </a>
        )}
      ...
      {/* Navigation Buttons */}
      <div className="hidden md:flex absolute right-0 top-2 space-x-2">
        <Button
          variant="outline"
          size="icon"
          className={cn(
      ...
      >
        {books.map((book) => (
          <div 
            key={book.id} 
            className="w-36 md:w-48 lg:w-56 flex-shrink-0 snap-start"
          >
            <BookCard {...book} />
          </div>
        ))}
